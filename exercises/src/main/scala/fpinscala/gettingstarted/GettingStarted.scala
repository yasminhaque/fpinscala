...
object MyProgram:
  ...
  def tailRecursiveFib(n: Int): Int =
    @annotation.tailrec
    def loop(iteration: Int, current: Int, previous: Int): Int =
      if n == iteration then current
      else loop(iteration + 1, current + previous, current)
    if n == 0 then 0 else
    loop(1, 1, 0)
